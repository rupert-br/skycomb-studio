// Shared with the backend's contract in app/backend/main.py — keep in sync by hand.
...
export interface CountryPeaks {
  country: string
  /** ISO 3166-1 alpha-2, used to render a flag emoji — see countryFlag() in mountains.ts. */
  code: string
  peaks: Peak[]
}

/** A free-text search match from the backend's Nominatim-backed geocoder. */
